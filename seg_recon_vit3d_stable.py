...
# ExtraDecoder: takes x_mean and x_std, outputs fft_ymin and fft_ymax
class ExtraDecoder(nn.Module):
    def __init__(self):
        super().__init__()

        # Small MLP: input is 2D (mean, std), output is 2D (fft_ymin, fft_ymax)
        self.mlp = nn.Sequential(
            nn.Linear(2, 16),     # First hidden layer
            nn.ReLU(inplace=True),
            nn.Linear(16, 2)      # Output layer: 2 outputs (ymin, ymax)
        )

    def forward(self, x_mean: torch.Tensor, x_std: torch.Tensor):
        """
        Inputs:
            x_mean: Tensor of shape (B,)
            x_std: Tensor of shape (B,)
        Output:
            Tensor of shape (B, 2): [fft_ymin, fft_ymax]
        """
        # Concatenate mean and std into shape (B, 2)
        stats = torch.stack([x_mean, x_std], dim=1)

        # Predict [fft_ymin, fft_ymax]
        result = self.mlp(stats)
        ymin, ymax = result[:,0], result[:,1]
        #ymin, ymax = self.mlp(stats)
        return ymin, ymax
...
class SegRecon_ViT_3D_StableTrain(nn.Module):
    def __init__(self, C_input=31, total_channels=61, patch_size=32, emb_size=768, num_heads=12,xminmax = False,ifft=True):
        ...
        self.minmaxdecoder = ExtraDecoder()
    ...
    def forward(self, x_input: torch.Tensor):
        ...
        x = self.out_conv(x)
        #print(x.size)
        x = torch.clip(x,0.00001,1)

        # Reshape ymin and ymax for broadcasting: (B, 1, 1, 1)
        ymin = ymin.view(B, 1, 1, 1)
        ymax = ymax.view(B, 1, 1, 1)

        # Scale from [0, 1] to [ymin, ymax]
        x = x * (ymax - ymin) + ymin
...
if __name__ == "__main__":
    ...
    model = SegRecon_ViT_3D_StableTrain()
