...
import torch
from monai.networks.blocks import UnetrUpBlock, UnetrPrUpBlock
...
#Decoder module to generate a binary segmentation mask from ViT-encoded features
class SegDecoder(nn.Module):
    def __init__(self,
        ...
        # First layer: project ViT embedding to conv feature map
        ...
        # Final 1x1 conv + sigmoid to output binary mask
    ...
    def forward(self, x: torch.Tensor, h: int, w: int) -> torch.Tensor:
        # Reshape from (B, T, E) -> (B, E, h, w)
        B, T, E = x.shape
        x = x.transpose(1, 2).contiguous().view(B, E, h, w)

        # Run full upsampling stack
        mask = self.decoder(x)  # (B, 1, H, W)

        return mask


# Main Vision Transformer-based segmentation/reconstruction model
class SegRecon_ViT_3D_DualTask(nn.Module):
    def __init__(self, C_input=31, total_channels=61, patch_size=32, emb_size=768, num_heads=12,ifft=False):
        ...
        self.seg_decoder = SegDecoder(embed_dim=emb_size)
    ...
    def forward(self, x_input: torch.Tensor):
        ...
        mask = self.seg_decoder(x,h,w) #Segmentation decoder
...
if __name__ == "__main__":
    ...
    model = SegRecon_ViT_3D_DualTask()
