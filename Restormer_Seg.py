...
class Encoder(nn.Module): 
    def __init__(self, inp_channels, dim, ffn_expansion_factor, heads, bias, LayerNorm_type, num_blocks):
...
class RecDecoder(nn.Module): 
    def __init__(self, dim, ffn_expansion_factor, heads, bias, LayerNorm_type, num_blocks, num_refinement_blocks):
...
class SegDecoder(nn.Module):
    def __init__(self, in_channels, num_classes, num_blocks):
        ...
        self.seg_layers = nn.Sequential(*layers)
...
class Restormer_Seg(nn.Module):
    ...
    def forward(self, inp_img):
        ...
        hb, wb = 8, 8  # To be divisible by 8
        ...
        # If input and output channels match, apply residual connection
        if out.shape[1] == inp_img.shape[1]:
            out = out + inp_img

        # Crop back to original size (in case padded)
        out = out[:, :, :h_inp, :w_inp]
